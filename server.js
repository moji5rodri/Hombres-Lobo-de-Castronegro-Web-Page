...
function makeRoom(code, hostId) {
  return {
    code,
    hostId,
    players: [],       // { id, nickname, role, alive, protected, poisoned }
...
function assignRoles(room) {
  ...
  // Wolves: roughly 1 per 4 players, min 1
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', ({ nickname }) => {
    ...
    rooms[code].players.push({ id: socket.id, nickname, role: null, alive: true, protected: false, poisoned: false });
    socket.join(code);
  ...
  socket.on('joinRoom', ({ nickname, code }) => {
    ...
    room.players.push({ id: socket.id, nickname, role: null, alive: true, protected: false, poisoned: false });
    socket.join(code);
    socket.emit('roomJoined', { code });
    broadcast(room);
    narrator(room, `👤 <b>${nickname}</b> se unió a la sala.`);
  });
  ...
  socket.on('wolfVote', ({ targetId }) => {
    ...
    room.nightVotes[socket.id] = targetId;
  ...
  socket.on('hunterShoot', ({ targetId }) => {
    ...
    if (room.hunterId !== socket.id) return;
    clearTimer(room, 'hunter');
    const target = getPlayer(room, targetId);
  ...
  socket.on('skipToVote', () => {
    const room = getRoomOf(socket.id);
    if (!room || room.state !== 'day' || room.hostId !== socket.id) return;
    clearTimer(room, 'day');
    startVoting(room);
  });
