...
function makeRoom(code, hostToken) {
  return {
    ...
    createdAt: Date.now(),
    timings: {
      dayChat: 90,    // segundos de charla de día
      voting: 30,     // segundos de votación
      hunter: 20,     // segundos del cazador para disparar
      nightStep: 45,  // segundos de cada paso nocturno (vidente, bruja, salvador…)
      wolves: 90,     // segundos de los lobos para votar
    },
  };
}
...
// ─── Role sets & counts ───────────────────────────────────────────────────────
const ROLE_ORDER_NIGHT = ['cupido', 'ladron', 'vidente', 'salvador', 'lobos', 'bruja'];
...
function assignRoles(room) {
  ...
  // Special roles (one each if enabled) — nina eliminada del juego
  const specials = ['cupido', 'ladron', 'vidente', 'bruja', 'cazador', 'alguacil', 'salvador'];
...
function nextNightStep(room) {
  ...
  if (step === 'bruja') {
    ...
    if (witch && witch.socketId) {
      const sock = io.sockets.sockets.get(witch.socketId);
  ...
  const timeout = (step === 'lobos' ? room.timings.wolves : room.timings.nightStep) * 1000;
...
function triggerHunter(room, hunterId) {
  ...
  const t = room.timings.hunter * 1000;
  room.deadline = Date.now() + t;
  narrator(room, `🏹 El Cazador ha caído — tiene ${room.timings.hunter} segundos para llevarse a alguien con él.`);
  broadcast(room);
...
function startDay(room) {
  room.state = 'day';
  room.dayVotes = {};
  const t = room.timings.dayChat * 1000;
  room.deadline = Date.now() + t;
  narrator(room, `☀️ Fase de día — Debatan y acusen. La votación comenzará en ${room.timings.dayChat} segundos.`);
  broadcast(room);
  clearTimer(room, 'day');
...
function startVoting(room) {
  room.state = 'voting';
  room.dayVotes = {};
  const t = room.timings.voting * 1000;
  room.deadline = Date.now() + t;
  narrator(room, `🗳️ ¡Votación! Tienen ${room.timings.voting} segundos para votar a quién ejecutar.`);
  broadcast(room);
...
io.on('connection', (socket) => {
  ...
  socket.on('setTimings', ({ timings }) => {
    const room = getRoomOf(myToken());
    if (!room || room.hostId !== myToken()) return;
    // Validar rangos razonables para cada tiempo
    const clamp = (v, min, max) => Math.max(min, Math.min(max, Math.round(v)));
    room.timings = {
      dayChat:   clamp(timings.dayChat   ?? room.timings.dayChat,   15, 300),
      voting:    clamp(timings.voting    ?? room.timings.voting,     10, 120),
      hunter:    clamp(timings.hunter    ?? room.timings.hunter,     10, 60),
      nightStep: clamp(timings.nightStep ?? room.timings.nightStep,  15, 120),
      wolves:    clamp(timings.wolves    ?? room.timings.wolves,     20, 180),
    };
    broadcast(room);
  });
