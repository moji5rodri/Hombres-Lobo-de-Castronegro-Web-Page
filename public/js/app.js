...
function goToMenuWithNickname(v){
  myNickname = v;
  sessionStorage.setItem('castronegro_nickname', v);
...
// Si ya hay un apodo guardado en esta pestaña (sessionStorage), saltamos directo al menú
(function restoreNickname(){
  const saved = sessionStorage.getItem('castronegro_nickname');
  if (saved) goToMenuWithNickname(saved);
})();
...
socket.on('roomCreated', ({ code }) => { myRoomCode = code; isHost = true; showScreen('screen-lobby'); });
socket.on('roomJoined', ({ code }) => { myRoomCode = code; isHost = false; showScreen('screen-lobby'); });
socket.on('error', (msg) => {
